...
import com.iteamcn.AutoLearn.weblearn.icve.api.entity.StudingInfo;
...
public class StudingHandler extends ApiHandler {

	public StudingHandler(HttpHandler httpHandler) {
		super(httpHandler);
		// TODO 自动生成的构造函数存根
	}
	...
	public InnerMessage doHttp(String arg,boolean withoutArgHead) throws Exception {
		...
		try{
			if(withoutArgHead){
				message=httpHandler.post(Url.POST_STUDING_COURSE, "userid="+arg, ContentType.FORM, Encoding.UTF8);
			}else{
				message=httpHandler.post(Url.POST_STUDING_COURSE, arg, ContentType.FORM, Encoding.UTF8);
			}
			
			if(message.getContentType().equals(ContentType.JSON)){
				newMessage.setObject(StudingInfo.getCourseList((JSONObject)message.getContent()));
